...
import { printBanner, printScanResult, printCleanResult, printDryRunResult, printError, LOBSTER, formatBytes, } from './utils.js';
...
program
    ...
    const cleanResult = await cleanItems(result.items, (item, index, total) => {
        cleanSpinner.text = chalk.cyan(`Shedding cruft... (${index + 1}/${total}) ${formatBytes(item.size)}`);
    });
    cleanSpinner.stop();
